 ...
 */
class Matrixratemodified extends \Magento\Framework\Model\ResourceModel\Db\AbstractDb
{
    /**
    ...
    public function getRate(\Magento\Quote\Model\Quote\Address\RateRequest $request, $zipRangeSet = false)
    {
        ...
        for ($j=0; $j<8; $j++) {
            ...
            if($request->getConditionMRName() =='package_weight_price') {
                ...
                $bind[':condition_value'] = $request->getPackageWeight();
                $bind[':condition2_value'] = $request->getPackageValue();
    ...
    protected function _getImportRow($row, $rowNumber = 0)
    {
        ...
        if($this->importConditionName=='package_weight_price') {

            // validate condition from value
            $value2From = $row[7] == '*' ? -1 : $this->_parseDecimalValue($row[7]);
            if ($value2From === false) {
                $this->importErrors[] = __(
                    'Please correct %1 From "%2" in Row #%3.',
                    $this->getConditionFullName($this->importConditionName),
            ...
            // validate conditionto to value
            $value2To = $row[8] == '*' ? 10000000 :$this->_parseDecimalValue($row[8]);
            if ($value2To === false) {
                $this->importErrors[] = __(
                    'Please correct %1 To "%2" in Row #%3.',
                    $this->getConditionFullName($this->importConditionName),
    ...
    protected function _saveImportData(array $data)
    {
        if (!empty($data)) {
            ...
            if('package_weight_price' == $this->importConditionName) {
